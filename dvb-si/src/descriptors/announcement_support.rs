    ...
    fn parse(bytes: &'a [u8]) -> Result<Self> {
        if bytes.len() < HEADER_LEN {
            return Err(Error::BufferTooShort {
                need: HEADER_LEN,
                have: bytes.len(),
                what: "AnnouncementSupportDescriptor header",
            });
        }
        if bytes[0] != TAG {
            return Err(Error::InvalidDescriptor {
                tag: bytes[0],
                reason: "unexpected tag for announcement_support_descriptor",
            });
        }
        ...
        if bytes.len() < end {
            return Err(Error::BufferTooShort {
                need: end,
                have: bytes.len(),
                what: "AnnouncementSupportDescriptor body",
            });
        }
        if length < INDICATOR_LEN {
            return Err(Error::InvalidDescriptor {
                tag: TAG,
